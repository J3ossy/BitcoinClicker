...
public class Draw_Upgrades extends JLabel {
	...
	protected void paintComponent(Graphics g) {
		...
		if (Gui.bUpgrades.isActive()) {
			...
			for (int i = 0; i < Gui.upgrade.length; i++) {
				...
				if (Gui.upgrade[i].getY() > 100) {
					...
					g.drawString(Gui.upgrade[i].getName(),
							(gui.width / 2) + (((gui.width / 2) / 3) / 2) - textWidth / 2, Gui.upgrade[i].getY() + 110);
					...
					g.drawString(s + " BTC", (int) ((gui.width / 2) + ((gui.width / 2) * (5.0 / 6.0)) - textWidth),
					...
					if (Gui.ugButton[i].isHover()) {
						...
						g.fillRect(Gui.ugButton[i].getX(), Gui.ugButton[i].getY(), Gui.ugButton[i].getWidth(),
								Gui.ugButton[i].getHeight());
					}
		...
		// Text Button UpgradeClicker
		g.setColor(Color.WHITE);
		textWidth = g.getFontMetrics().stringWidth("Upgrade");
		g.drawString("Upgrade", gui.width / 4 - textWidth / 2, 532);

		// Textanzeige Clickercost
		String s = df.format(BitcoinMain.ugClickerCost) + " BTC";
		textWidth = g.getFontMetrics().stringWidth(s);
		g.drawString(s, gui.width / 4 - textWidth / 2, 480);

		g.setFont(new Font("Arial", Font.PLAIN, 22));
		s = df2.format(BitcoinMain.bpc) + " BTC/c";
		textWidth = g.getFontMetrics().stringWidth(s);
